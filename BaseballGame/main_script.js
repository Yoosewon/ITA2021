...
function settingGame(flag) {
    ...
    if(ditc_mode === '재시도') {
        ...
        fn_changeMsg("main_title", "게임중");
    } else if(ditc_mode === "게임중") {
    ...
    //console.log(random_pick_val[0]);
    //console.log(random_pick_val[1]);
    //console.log(random_pick_val[2]);
    //console.log(random_pick_val[3]);
}

function confirm_answer () {
    var ditc_mode = document.getElementById('main_title_msg').textContent;
    if("게임중" != ditc_mode) {
        alert('먼저 게임을 시작해주세요');
        return;
    }
    ...
    if(strike_cnt == 4) {
        ...
        var message = document.createTextNode(insertVal0+insertVal1+insertVal2+insertVal3 + " :: " + msg);
